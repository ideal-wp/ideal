...
if (class_exists('Kirki')) {
    ...
    $ideal_is_sidebar           = $ideal_options['is_post_sidebar'] ;
        ...
        $ideal_is_cards             =  $ideal_options['post-cards'] ;
    }
}else{
...
if (!empty($ideal_is_cards) && $ideal_is_cards == true ) {
    ...
    $ideal_card = 'uk-ideal card-default';
}
...
if (!empty($ideal_options['repeat-scrollspy']) && $ideal_options['repeat-scrollspy'] == true) {
    $ideal_animation_repeat = 'true';

} elseif (!empty($ideal_options['repeat-scrollspy']) && $ideal_options['repeat-scrollspy'] == false) {
    $ideal_animation_repeat = 'false';
}

if (!empty($ideal_options['repeat-scrollspy']) && $ideal_options['add-scrollspy'] == true) {

    $ideal_animation = 'uk-scrollspy="cls: ' . $ideal_animation_class . '; target: .uk-ideal_card; delay: 500; repeat: ' . $ideal_animation_repeat . '"';

}



if (!empty( $ideal_is_page_sidebar ) && $ideal_is_page_sidebar == true ) {
...
if (!empty($ideal_sidebar_position) && $ideal_sidebar_position == 'left') {
    $ideal_flex = 'uk-ideal_flex-first@m';
} else {
    $ideal_flex = null;
}
...
if (!empty($ideal_is_sidebar) && $ideal_is_sidebar == true) {
  $ideal_id_container = 'uk-container-expand';
...
if ($ideal_is_featured_image == true  &&  $ideal_is_cards == true  ){
