...
$ideal_is_trans = get_post_meta(get_the_ID(), 'is_header_trans', true);
$home_url = esc_url(home_url());
$logo_site_name = get_bloginfo('name');

if (!empty($ideal_options['logo-ss-height'])) {
    $logo_height = $ideal_options['logo-ss-height'];
} else {
    $logo_height = '35px';
}
...
} elseif (!empty($ideal_options['logo-img-menu'])) {
